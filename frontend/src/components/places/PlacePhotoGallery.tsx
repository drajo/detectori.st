...
export const PlacePhotoGallery: React.FC<PlacePhotoGalleryProps> = ({ placeId, photos, onPhotosChange }) => {
  ...
  const handleUpload = useCallback(async (file: File) => {
    ...
    setIsUploading(true);
    try {
      const newPhoto = await placesService.uploadPhoto(placeId, file);
  ...
  }, [placeId, onPhotosChange]);

  // Window-level paste listener — łapie zrzuty ekranu z clipboardu
  useEffect(() => {
  ...
  const handleSetCover = async (photoId: string) => {
    try {
      const updatedPhoto = await placesService.setCoverPhoto(placeId, photoId);
          ...
          {photos.map((photo) => (
            <div key={photo.id} className="relative rounded-xl overflow-hidden border border-explorer-border group aspect-square">
              <a href={photo.url} target="_blank" rel="noopener noreferrer" className="block w-full h-full">
                <img src={photo.thumbnailUrl} alt="Place photo" className="w-full h-full object-cover" loading="lazy" />
              </a>
              {photo.isCover && (
              ...
              <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-end justify-between p-2 gap-1 pointer-events-none">
                {!photo.isCover && (
                  <button
                    type="button"
                    onClick={(e) => { e.preventDefault(); void handleSetCover(photo.id); }}
                    aria-label="Set as cover"
                    title="Set as cover"
                    className="pointer-events-auto flex items-center justify-center w-7 h-7 rounded-lg bg-explorer-gold hover:bg-yellow-400 text-black transition-colors focus:outline-none focus:ring-2 focus:ring-explorer-gold"
                  >
                ...
                <button
                  type="button"
                  onClick={(e) => { e.preventDefault(); void handleDelete(photo.id); }}
                  aria-label="Delete photo"
                  title="Delete photo"
                  className="pointer-events-auto flex items-center justify-center w-7 h-7 rounded-lg bg-explorer-danger hover:bg-red-500 text-white transition-colors focus:outline-none focus:ring-2 focus:ring-explorer-danger"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-3.5 h-3.5" aria-hidden="true">
          ...
          {isUploading ? (
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              <p className="text-sm text-explorer-text-secondary">
                Drop photo, paste with <kbd className="px-1.5 py-0.5 rounded bg-explorer-bg border border-explorer-border text-xs font-mono">Ctrl</kbd>+<kbd className="px-1.5 py-0.5 rounded bg-explorer-bg border border-explorer-border text-xs font-mono">V</kbd>, or <span className="text-explorer-accent">browse</span>
              </p>
